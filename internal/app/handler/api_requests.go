...
func (h *Handler) ApiListRequests(ctx *gin.Context) {
	...
	// Плоский DTO без вложенных объектов
	...
	resp := make([]requestDTO, 0, len(list))
	for _, r := range list {
		cnt, _ := h.Repository.CountRequestSymptomsWithComment(r.ID)
		var modID *uint
		var modLogin *string
		if r.Moderator != nil {
			modID = &r.Moderator.ID
			l := r.Moderator.Login
			modLogin = &l
		}
		resp = append(resp, requestDTO{
			ID:                 r.ID,
			Status:             r.Status,
			FormedAt:           r.FormedAt,
...
func (h *Handler) ApiGetRequest(ctx *gin.Context) {
	...
	// Формируем плоский DTO без вложенных блоков
	...
	type requestDetailedDTO struct {
		ID                 uint          `json:"id"`
		Status             string        `json:"status"`
		FormedAt           *time.Time    `json:"formed_at"`
		CompletedAt        *time.Time    `json:"completed_at"`
	...
	// dr содержит сложную структуру с заявкой и симптомами
	// Сформируем облегченную
	var modID *uint
	var modLogin *string
	if dr.Request.Moderator != nil {
		modID = &dr.Request.Moderator.ID
		l := dr.Request.Moderator.Login
		modLogin = &l
	}
	// Собираем симптомы
	symptoms := make([]symptomItem, 0, len(dr.Symptoms))
	for _, s := range dr.Symptoms {
		symptoms = append(symptoms, symptomItem{
			ID:          s.ID,
			Title:       s.Title,
			Category:    s.Category,
	...
	dto := requestDetailedDTO{
		...
		UserID:             dr.Request.User.ID,
		UserLogin:          dr.Request.User.Login,
		ModeratorID:        modID,
		ModeratorLogin:     modLogin,
		DoctorComment:      dr.Request.DoctorComment.String,
		Symptoms:           symptoms,
	}
	jsonResponse(ctx, dto, 1, gin.H{"id": id})
}
